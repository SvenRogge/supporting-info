...
if __name__=='__main__':
    ...
    system = System.from_file("../DUT49_vol45400_cp.chk")
...
# Check reference trajectory energies
    ...
    restart=RestartWriter(restartf,step=5000,flush=10)
    thermo = NHCThermostat(temp, timecon=1000.0*femtosecond)
    baro = MTKBarostat(ff, temp, press, timecon=2000.0*femtosecond, vol_constraint=True)
    TBC = TBCombination(thermo, baro)
    vsl = VerletScreenLog(step=100)
# NPT (P=0)
    hooks = [TBC,vsl, hdf5,restart]
    md = VerletIntegrator(ff, timestep, hooks=hooks)
    md.run(1000000) # 500 ps
