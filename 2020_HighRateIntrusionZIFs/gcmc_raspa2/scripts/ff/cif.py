...
def dump_cif(filename, data, ffatypes=None, charges=None):
    '''Write a CIF file
    ...
    symbols = [periodic[data.numbers[i]].symbol for i in xrange(data.natom)]
    if ffatypes == None:
        ffatypes = [symbols[i]+str(i+1) for i in xrange(data.natom)]
    assert len(ffatypes)==data.natom
    with open(filename, 'w') as f:
        ...
        if charges is not None:
            print >> f, '_atom_site_charge'
        for i in xrange(data.natom):
            fx, fy, fz = data.cell.to_frac(data.coordinates[i])
            if charges is not None:
                print >> f, '%10s %3s % 12.6f % 12.6f % 12.6f % 12.6f' % (ffatypes[i], symbols[i], fx, fy, fz, charges[i])
            else:
                print >> f, '%10s %3s % 12.6f % 12.6f % 12.6f' % (ffatypes[i], symbols[i], fx, fy, fz)
...
def load_cif(filename, cell=None):
    '''Load a CIF file
    ...
    if 'symmetry_equiv_pos_as_xyz' in fields.keys(): key = 'symmetry_equiv_pos_as_xyz'
    elif 'space_group_symop_operation_xyz' in fields.keys(): key = 'space_group_symop_operation_xyz'
    else: raise NotImplementedError
    generators = [equiv_pos_to_generator(s) for s in fields[key]]
    ...
    symmetry = Symmetry(name, generators, prim_fracs, prim_numbers, cell, None)
