...
from yaff import System, ForceField, HDF5Writer, RestartWriter, XYZWriter,\
    ...
    PairPotLJ, PairPotMM3, ForcePartPair, swap_noncovalent_lammps, write_lammps_table
...
if __name__=='__main__':
    ...
    # Ghost ff only with Yaff, for testing purposes
    ghostff_yaff = GhostForceField(ff, ghost_indexes, write_ghost_pos, write_ghost_gpos)

    # Call separately write_lammps_table to make minimal r to 0.10 A
    ...
    print('Yaff energy')
    ghostff_yaff.compute()
    print(ghostff_yaff.compute()/kjmol)
    for part in ghostff_yaff.ff.parts:
